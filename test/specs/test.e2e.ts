...
describe("Calculator", () => {
  it("Verify background color change functionality", async () => {
    await entry.mainScreen.clickElement(
      entry.mainScreen.elements.moreOptionsButton
    );
    await entry.mainScreen.clickElement(
      entry.mainScreen.elements.changeThemeButton
    );
    await entry.mainScreen.clickElement(
      entry.themeScreen.elements.darkThemeButton
    );
    await entry.mainScreen.clickElement(
      entry.themeScreen.elements.submitThemeButton
    );
    ...
    const darkThemeButton = await entry.mainScreen.getElement(
      entry.themeScreen.elements.darkThemeButton
    );
  ...
  it("Verify the multiply functionality", async () => {
    ...
    await entry.mainScreen.clickElement(
      entry.mainScreen.elements.multiplyButton
    );
    ...
    const resultText = await entry.mainScreen.getElementText(
      entry.mainScreen.elements.finalResultField
    );
  ...
  it("The error message shown during the dividing by zero", async () => {
    ...
    await entry.mainScreen.clickElement(entry.mainScreen.elements.equalsButton);
    const resultText = await entry.mainScreen.getElementText(
      entry.mainScreen.elements.resultPreviewField
    );
  ...
  it("History clearing functionality", async () => {
    ...
    await entry.mainScreen.clickElement(
      entry.mainScreen.elements.historyButton
    );

    await entry.mainScreen.clickElement(
      entry.historyScreen.elements.moreHistoryOptionsButton
    );
    await entry.mainScreen.clickElement(
      entry.historyScreen.elements.clearHistoryButton
    );
    await entry.mainScreen.clickElement(
      entry.historyScreen.elements.submitButton
    );
    ...
    await expect(
      await entry.historyScreen.isElementDisplayed(
        entry.historyScreen.elements.noHistoryText
      )
    ).toBe(true);
  });
});
